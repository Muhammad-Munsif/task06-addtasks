...
class TaskManager {
  ...
  toggleTask(id) {
    ...
    if (task) {
      ...
      this.updateStats();
      this.showToast(
        `Task marked as ${task.done ? "completed" : "pending"}!`,
        "success"
      );
    }
  }
  ...
  toggleImportant(id) {
    ...
    if (task) {
      ...
      this.updateStats();
      this.showToast(
        `Task marked as ${task.important ? "important" : "normal"}!`,
        "success"
      );
    }
  }
  ...
  deleteSelected() {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${this.selectedTasks.size} task(s)?`
      )
    )
      return;
  ...
  updateBulkActions() {
    ...
    document.getElementById("selectAll").checked =
      this.selectedTasks.size === this.filteredTasks().length;
  }
        ...
        (task) => `
          <li class="task-item ${task.done ? "done" : ""}" data-id="${task.id}">
            ...
            <div class="task-actions">
              ...
              <button class="action-btn edit-btn" onclick="taskManager.editTask(${
                task.id
              })">
